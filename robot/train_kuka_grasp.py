    ...
    for i_episode in range(num_episodes):
        ...
        for t in count():
            ...
            if not done:
                ...
                next_stacked_states_t = torch.cat(
                    tuple(next_stacked_states), dim=1)
            else:
                next_stacked_states_t = None

            # store the transition in memory
            memory.push(stacked_states_t, action,
                        next_stacked_states_t,
                        reward)
            ...
            if done:
                ...
                if (best_mean_reward is None or
                    best_mean_reward < mean_reward) and \
                   i_episode > 100:
                    ...
                    if best_mean_reward is not None:
                        print("Best mean reward updated %.1f -> %.1f, \
                               model saved"
        ...
        if i_episode % 10 == 0:
            writer.add_scalar('ten episodes average rewards',
                              ten_rewards/10.0, i_episode)
            ten_rewards = 0
